    ...
    static void indexDoc(IndexWriter writer, Path file, long lastModified) throws IOException {
        try (InputStream stream = Files.newInputStream(file)) {
            ...
            try {
                ...
                doc.add(new StringField("title", parser.title.trim(), Field.Store.YES));
                doc.add(new TextField("contents", parser.body, Field.Store.YES));
                //Parser requires a catch exception
            } catch(SAXException | IOException a) {
                System.out.println(" caught a " + a.getClass() +
                        "\n with message: " + a.getMessage());
            }
